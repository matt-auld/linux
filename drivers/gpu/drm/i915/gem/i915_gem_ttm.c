			 ...
			 struct ttm_place *hop)
{
	...
	new_iter = new_man->use_tt ?
		...
		ttm_kmap_iter_iomap_init(&_new_iter.io, &new_reg->iomap,
					 new_st, new_reg->region.start);

	old_iter = old_man->use_tt ?
		ttm_kmap_iter_tt_init(&_old_iter.tt, bo->ttm) :
		ttm_kmap_iter_iomap_init(&_old_iter.io, &old_reg->iomap,
...
static void i915_ttm_bo_destroy(struct ttm_buffer_object *bo)
{
	struct drm_i915_gem_object *obj = i915_ttm_to_gem(bo);
